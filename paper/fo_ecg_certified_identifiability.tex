...
Figures~\ref{fig:identifiability-ladder-main} and
\ref{fig:chain-main} make the methodological distinction explicit: the first
separates one-rate nonidentifiability, two-rate recovery, and three-rate model
checking; the second separates a fitted point from a response-disk coverage
certificate and a shared-parameter feasible set.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/method_identifiability_geometry.pdf}
\caption{Identifiability ladder for the declared exact harmonic surrogate.
(a) At one rate, every candidate order and damping can reproduce the same
response through a candidate-dependent morphology, so a numerical point is
not structural identification.  (b) With two exact rates and one shared
nonzero complex morphology, the complete quotient removes the common complex
gain and has a unique physical inverse.  The displayed rays use rate ratio
$\rho=2$; the theorem holds for every $\rho>1$.  (c) A third geometrically
spaced rate supplies a redundant cross-ratio equation rather than another
fitted nuisance.  These panels concern exact structural geometry under
Assumption~\ref{ass:boundary-main}; they do not establish finite-record
coverage or a physiological fractional mechanism.}
\label{fig:identifiability-ladder-main}
\end{figure}

Figure~\ref{fig:identifiability-ladder-main} summarizes the information
hierarchy: one rate leaves an equivalence fibre, two rates collapse it under
the common-gauge contract, and a third rate creates a falsifiable equation.
Finite-record uncertainty is addressed separately below.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/certification_workflow_comparison.pdf}
\caption{Conceptual point-estimation and certified-set paths; the upper path
is illustrative and is not an algorithm comparison performed in this paper.
A point WLS estimate or point optimizer/filter output does not by itself
enclose acquisition, numerical, and model errors.  The proposed path binds
exact sample indices and hashes, replays the WLS centre and validated
numerical disk, and sends independently sourced primitives through the R2
coverage gate.  A pass forms response disks for shared-parameter joint
inversion; missing evidence returns \status{NOT\_CERTIFIABLE}, while a protocol
outside the declared information boundary is excluded.  R2 pass means
coverage, not model acceptance.  \status{REJECT\_MODEL} would require a
complete outer-set emptiness proof; the current real-data pilot has no R2
pass.}
...
Figure~\ref{fig:core-method-evidence-main} aligns the deterministic evidence
with four distinguishing method claims: gauge-invariant recovery, redundant
falsification, a fail-closed response disk, and explicit finite-error
resolution cost.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.98\linewidth]{figures/core_method_evidence.pdf}
\caption{Evidence matched to the distinguishing method claims.  (a) Absolute
order error before and after a common complex gain over 1,260 exact two-rate
cases spanning the frozen rate ratios, damping decades, and morphology pairs.
(b) Truth-referenced geometric residuals for admissible triples and controls:
the control replaces only the third response by one from a separately valid
$(1,3)$ pair with 5\% changed damping and morphology re-anchored to preserve
the first response.  Every admissible triple is consistent and every control
is detected.  (c) The sealed exact-sample fixture contains its known response,
while either a payload change or an unsealed radius change returns
\status{NOT\_CERTIFIABLE}.  (d) A conservative legacy fixed-subset outer
diagnostic retains coverage but widens as the response radius grows; it is not
the exact joint R1 projection.  These panels audit implementations and their
resolution cost on exact or synthetic domains; they are not proofs,
noisy-data validation, real-ECG R2 calibration, or evidence for a physiological
fractional mechanism.}
\label{fig:core-method-evidence-main}
\end{figure}
...
Figure~\ref{fig:core-method-evidence-main}(c) shows the known response in units
of the final certified radius and the two fail-closed seal controls.
...
proof and not noisy-data evidence; panels (a)--(b) of
Figure~\ref{fig:core-method-evidence-main} visualize the recovery accuracy and
the separation between valid and deliberately inconsistent triples.
...
Figure~\ref{fig:core-method-evidence-main}(d) places that loss of resolution on
the same radius axis: unlike a point output, the outer set visibly reports the
price of finite error instead of retaining false precision.
...
Figure~\ref{fig:core-method-evidence-main} audits the method on declared exact
and synthetic domains.  The present subsection separately reports the
real-data negative pilot and does not reuse its residuals as R2 bounds.
...
As Figure~\ref{fig:identifiability-ladder-main} emphasizes, the contribution
is not an additional fractional filter but a change in information structure.
